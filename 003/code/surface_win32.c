...
#include "surface.h"
	...
	{	
		// Surface Width et Height minimum 1..MAX px. 
		SURFACE.Width  = CLAMP(1, Width,  SURFACEMAXWIDTH);
		SURFACE.Height = CLAMP(1, Height, SURFACEMAXHEIGHT);
	
		// Caluler Memoire a allouer
		u16 BytesPerPixel = 4;
		u32 MemorySize = Align16(SURFACE.Width * SURFACE.Height * BytesPerPixel);
	...
	void SetSizeSurface(void)
	{
		...
		SURFACE.Width  = (u16)(clientRect.right - clientRect.left);
		SURFACE.Height = (u16)(clientRect.bottom - clientRect.top);
	}
